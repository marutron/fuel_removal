...
1. Файл `initial_state` - топазовский файл начального состояния.\
...
#### Результат работы
В результате работы скрипта в директории `result` проекта создаются:\
1. Файл `result.txt`, содержащий распределение ТВС в контейнерах.
```commandline
...
2. Файл состояния ТОПАЗ после всех перестановок для отправки - `final_state`.\
3. Файл для работы машины перегрузочной - `mp_file.mp`.\
4. Файлы начальных и конечных картограмм отсеков БВ .\
5. Файлы с заполненными таблицами контейнеров.\
6. Файлы паспортов упаковки - в директории `Паспорта ТУК`.\
